...
`worker/src/index.ts` lines 33 to 39 declare the variables the Worker reads at runtime:
...
Every Wrangler command in this runbook runs from `worker/`. Wrangler 4.128.0 is pinned in
`worker/package.json` line 25 and installed only there. Run it from the repository root and
npx fetches a different version from the registry.
...
pinned 4.128.0. That help text also offers `--update-config`, which edits the Wrangler
...
**KV reads.** Each tick reads `idx` once and each device record once, so `1440 * (1 + N)`.
The free tier allows 100,000 reads a day (`REFERENCES.md` line 35). That gives `N <= 68`.
...
quota. `worker/src/index.ts` lines 176 to 187 state that arithmetic, and line 188
...
- Sending writes the record once per tick that sent anything (`worker/src/index.ts` lines
  336 to 338). At most 30 writes. Reminders that fire in the same minute share one write.
- Pruning an expired instant writes the record once more, on a later tick. At most 30
  writes. See `worker/src/schedule.ts` lines 308 to 314, gated by `pruneDelta` at
  `worker/src/index.ts` line 336.
...
- `idx` is written only when a tick removed a device or the reconcile found an orphan
  (`worker/src/index.ts` lines 343 to 345). Zero in steady state, at most 24 a day.
...
push. That is roughly `N <= 20` devices before a tick can trip the limit. The ceiling is
platform fact 6 of `docs/plans/2026-09-01-05-reminders.md`, sourced from the Workers limits
page recorded at `REFERENCES.md` line 34. Watch this number, not the daily quotas.

**Not verified.** Whether Cron Trigger invocations consume the 100,000 requests a day quota
(`REFERENCES.md` line 34). The limits page does not say. At 1,440 ticks a day it stays
inside the cap either way.

The Worker also caps itself:

- 100 devices, `worker/src/index.ts` line 52. It answers 503 past that.
- 200 reminders per device, `worker/src/schedule.ts` line 37.
- A 64 KiB request body, `worker/src/index.ts` line 60.

## 12. Enable reminders on the phone

<!-- TODO(P5-T8): rewrite once the panel ships -->

Not built yet. Task 8 of `docs/plans/2026-09-01-05-reminders.md` specifies the Settings
panel and `src/ui/reminders/` does not exist. The contract the plan sets:

- A Reminders section in Settings, with a status line and an **Enable reminders** toggle.
- A day of reminder time input, and a checkbox per lead time.
- An install guide in place of the toggle on iOS, whenever the page is not running as an
  installed app. The plan's `currentStatus` returns `needs-install` when
  `isIos() && !isInstalledPwa()`.
- A status line carrying exactly one of six strings, from `statusLine` in the plan's
  Task 8 Step 3:
  ...
  - `Reminders are active. Schedule last sent at HH:MM.`

Until it ships, the enablement order is the one the platform forces.

**iPhone and iPad, iOS 18.4 or later.** Install first, then enable.
...
4. Close Safari. Open the app from the Home Screen icon.
...
**Android, Chrome.** Enable directly. Installation is optional.
...
| `PUT` returns 413 | a request body over 64 KiB | `worker/src/index.ts` line 60 | reduce the number of reminders; 200 instants fit well inside the cap |
| `PUT` returns 503 | the device index is at the 100 device cap | `worker/src/index.ts` line 52 | remove dead devices, or raise the cap and redeploy |
...
on the pinned 4.128.0. Omit the id to roll back to the previous version.
