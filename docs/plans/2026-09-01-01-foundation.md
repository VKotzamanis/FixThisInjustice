...
`default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; font-src 'self'; connect-src 'self' https://*.workers.dev; frame-src https://invidious.nerdvpn.de https://inv.nadeko.net https://invidious.tiekoetter.com https://yt.chocolatemoo53.com https://inv.thepixora.com https://invidious.f5.si; worker-src 'self'; base-uri 'none'; form-action 'none'; object-src 'none'`.
...
grep -c 'frame-src https://invidious.nerdvpn.de https://inv.nadeko.net https://invidious.tiekoetter.com https://yt.chocolatemoo53.com https://inv.thepixora.com https://invidious.f5.si' dist/index.html
```
Expected:
```
    ...
    steps:
      ...
      - name: Build hygiene — no inline script, no CDN, CSP present
        run: |
          ...
          expected='frame-src https://invidious.nerdvpn.de https://inv.nadeko.net https://invidious.tiekoetter.com https://yt.chocolatemoo53.com https://inv.thepixora.com https://invidious.f5.si'
...
§3 originally named six hosts that overlapped `legacy/console-video.jsx`'s `INSTANCES` in only three places. That is now resolved in favour of the legacy list, which is the one the shipped app has actually been pointing users at: `invidious.nerdvpn.de`, `inv.nadeko.net`, `invidious.tiekoetter.com`, `yt.chocolatemoo53.com`, `inv.thepixora.com`, `invidious.f5.si`. `src/config/videoInstances.ts`, §3's CSP, this plan's Step 7 block, and the CI grep all carry that list in that order, so the generated policy matches the specification byte for byte. Before P4 ships the video modal, someone should still re-check the hosts — the security review confirmed only that six hostnames resolved in DNS on 2026-09-01, which says nothing about who controls them today.
